...
if [ $# -ne 4 ]; then
    echo "Four arguments required: schema file, data type mapping file, database type, and output file"
    exit 1
fi

# Read the schema.json file and data type mapping file
schema=$(cat $1)
data_type_mapping=$(cat $2)
database_type=$3
output_file=$4

# Use jq to parse the JSON and generate the SQL commands
echo -e "-- Create the schema" > $output_file
echo -e "CREATE SCHEMA IF NOT EXISTS $(echo $schema | jq -r .schema_name);\n" >> $output_file
...
for table in $(echo $schema | jq -r '.tables[] | @base64'); do
    ...
    echo -e -n "CREATE TABLE IF NOT EXISTS $(echo $schema | jq -r .schema_name).$table_name (\n" >> $output_file
    ...
    for column in $(echo $table | jq -r '.columns[] | @base64'); do
        ...
        data_type=$(echo $data_type_mapping | jq -r ".$database_type.$data_type")
