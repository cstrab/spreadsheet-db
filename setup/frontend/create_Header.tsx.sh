...
echo "import { NavLink } from 'react-router-dom';" >> $output_file
...
echo "          <NavLink to=\"/\" style={{ fontSize: '18px', color: 'white', textDecoration: 'none', marginRight: '20px', marginBottom: '2px' }}>" >> $output_file
echo "            Home" >> $output_file
echo "          </NavLink>" >> $output_file
echo "          <div onClick={handleMenuOpen} style={{ fontSize: '18px', cursor: 'pointer' }}>" >> $output_file
...
for table in $(echo $schema_file | jq -r '.tables[] | @base64'); do
    ...
    # Get the table name and format it for display
    ...
    # Add the table route to the Header.tsx file
    echo "            <MenuItem onClick={handleMenuClose}>" >> $output_file
    echo "              <NavLink to=\"/$table_path\">$display_name</NavLink>" >> $output_file
...
echo "      </AppBar>" >> $output_file
echo "      <Toolbar style={{ minHeight: '52px' }} />" >> $output_file
echo "    </>" >> $output_file
echo "  );" >> $output_file
echo "};" >> $output_file
